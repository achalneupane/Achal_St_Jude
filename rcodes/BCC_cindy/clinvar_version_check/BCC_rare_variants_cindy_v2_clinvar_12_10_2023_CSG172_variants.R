...
cc1 <- cbind.data.frame(clinvar.all$AF_nfe, clinvar.all$AF_afr, clinvar.all$AF, clinvar.all$ID)
...
## Extract same variants from bim file
bim <- fread("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic/plink_all/chr.ALL.Survivor_WES.GATK4180.hg38_biallelic_ID_updated.bim", header = F)
bim$KEY <- paste0("chr", bim$V1, ":", bim$V4)
bim <- bim[bim$KEY %in% kim_ST1.csg172.vars.unique$KEY,]

bim$REF <- bim$V6
bim$Effect_allele <- bim$V5
bim$REF_flipped <- chartr("acgtACGT", "tgcaTGCA", bim$REF)
bim$Effect_allele_flipped <- chartr("acgtACGT", "tgcaTGCA", bim$Effect_allele)
bim$KEY2 <- paste0(bim$KEY,":", bim$REF,":",bim$Effect_allele, ";", bim$KEY,":", bim$Effect_allele, ":", bim$REF, ";", bim$KEY,":", bim$REF_flipped,":",bim$Effect_allele_flipped, ";", bim$KEY,":", bim$Effect_allele_flipped, ":", bim$REF_flipped,";")
...
for (i in 1:nrow(bim)){
  ...
  for (k in 1:length(snps)){
    matching_row <- which(kim_ST1.csg172.vars.unique$SNP %in% snps)
  }
  # If a match is found, assign the value to bim$CSG_vars
  if (length(matching_row) > 0) {
    bim$CSG_var[i] <- kim_ST1.csg172.vars.unique$SNP[matching_row]
  }
}

write.table(bim$CSG_var, "Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/BCC/files_shared_by_cindy/analysis/clinvar_version_check/12_10_2023_CGS172/rare_variants_to_extract.txt", row.names = F, col.names = F, quote = F)
...
########################
library("data.table")
raw <- fread("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/BCC/files_shared_by_cindy/analysis/clinvar_version_check/12_10_2023_CGS172/all_BCC_rare_variants_recodeA.raw", header = T)
...
col.extract <- colnames(raw)[grepl("chr", colnames(raw))]
