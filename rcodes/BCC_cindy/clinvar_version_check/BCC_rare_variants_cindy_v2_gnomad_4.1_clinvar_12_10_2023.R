...
## 1. clinvar
clinvar <- read.delim("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/Survivor_WES/annotation/snpeff_round3_gnomad4.1/all_new_clinvar_P_LP.txt", header = T, sep = "\t", stringsAsFactors = F)
dim(clinvar)
...
dim(cc)
# 7528
write.table(cc, "Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/BCC/files_shared_by_cindy/analysis/clinvar_version_check/gnomad_4.1_12_10_2023/rare_variants_to_extract.txt", row.names = F, col.names = F, quote = F)
# Dear Achal,
#
...
########################
library("data.table")
raw <- fread("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/BCC/files_shared_by_cindy/analysis/clinvar_version_check/gnomad_4.1_12_10_2023/all_BCC_rare_variants_recodeA.raw", header = T)
...
## SJLIFE
table(carrier.status.sjlife$kim_ST1.csg60.clinvar.all.status)[2]/nrow(carrier.status.sjlife) 
table(carrier.status.sjlife$kim_ST1.csg172.clinvar.all.status)[2]/nrow(carrier.status.sjlife)
table(carrier.status.sjlife$BCC.panel.clinvar.all.status)[2]/nrow(carrier.status.sjlife) 
table(carrier.status.sjlife$clinvar.BCC.result.clinvar.all.status)[2]/nrow(carrier.status.sjlife) 
table(carrier.status.sjlife$NCI_table3.clinvar.all.status)[2]/nrow(carrier.status.sjlife) 

## CCSS
table(carrier.status.ccss$kim_ST1.csg60.clinvar.all.status)[2]/nrow(carrier.status.ccss) 
table(carrier.status.ccss$kim_ST1.csg172.clinvar.all.status)[2]/nrow(carrier.status.ccss)
table(carrier.status.ccss$BCC.panel.clinvar.all.status)[2]/nrow(carrier.status.ccss) 
table(carrier.status.ccss$clinvar.BCC.result.clinvar.all.status)[2]/nrow(carrier.status.ccss) 
table(carrier.status.ccss$NCI_table3.clinvar.all.status)[2]/nrow(carrier.status.ccss) 

## SJLIFE1 (2986 samples)
table(carrier.status.sjlife2986$kim_ST1.csg60.clinvar.all.status)[2]/nrow(carrier.status.sjlife2986) 
table(carrier.status.sjlife2986$kim_ST1.csg172.clinvar.all.status)[2]/nrow(carrier.status.sjlife2986)
table(carrier.status.sjlife2986$BCC.panel.clinvar.all.status)[2]/nrow(carrier.status.sjlife2986) 
table(carrier.status.sjlife2986$clinvar.BCC.result.clinvar.all.status)[2]/nrow(carrier.status.sjlife2986) 
table(carrier.status.sjlife2986$NCI_table3.clinvar.all.status)[2]/nrow(carrier.status.sjlife2986) 
