...
refPopLabels <- read.table('~/Work/1000G_phase_3/20130502/integrated_call_samples_v3.20130502.ALL.panel', header=TRUE)
...
pcs <- read.table('~/Work/WGS_SJLIFE/QC/pca/SJLIFE_and_1kGP_final_top_20_PCs.eigenvec', header=FALSE)
...
pcs1_2.label$pop[is.na(pcs1_2.label$pop)] <- "SJLIFE"
...
test = subset(pcs1_2.label, pop == "SJLIFE")
...
write.table(tmp.EUR, '~//Work/WGS_SJLIFE/QC/pca/SJLIFEWGS_EUR_based_on_1kGP_Phase_3_data.txt',row.names=FALSE,quote=FALSE)
...
write.table(tmp.AMR, '~/Work/WGS_SJLIFE/QC/pca/SJLIFEWGS_EUR_AMR_based_on_1kGP_Phase_3_data.txt',row.names=FALSE,quote=FALSE)
...
write.table(tmp.AFR, '~/Work/WGS_SJLIFE/QC/pca/SJLIFEWGS_AFR_based_on_1kGP_Phase_3_data.txt',row.names=FALSE,quote=FALSE)

# Create a PCA plot
#pdf('~/Work/WGS_SJLIFE/QC/pca/PCA_plot_SJLIFEWGS_1kGP.pdf')
pdf('~/Work/WGS_SJLIFE/QC/pca/PCA_plot_SJLIFEWGS_1kGP_AFR.pdf')
...
plot(pcs1_2.label_SJLIFE_EUR$PC1, pcs1_2.label_SJLIFE_EUR$PC2, main="SJLIFE WGS samples [Europeans alone]", col="black", bg=as.factor(pcs1_2.label_SJLIFE_EUR$pop), pch=21, cex=1.2, xlab="Principal Component 1", ylab="Principal Component 2")
...
plot(pcs1_2.label_SJLIFE_AFR$PC1, pcs1_2.label_SJLIFE_AFR$PC2, main="SJLIFE WGS samples [Africans alone]", col="black", bg=as.factor(pcs1_2.label_SJLIFE_AFR$pop), pch=21, cex=1.2, xlab="Principal Component 1", ylab="Principal Component 2")
