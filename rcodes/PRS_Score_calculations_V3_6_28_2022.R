...
# Fix variants with X in reference; I have manually checked the variants in the bim file and assigned them here
...
#######################################################################
## create bed files for the new variants after first PRS calculation ##
#######################################################################
second.batch.prs.vars <- read.table("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/prs/second_batch_search.txt", sep ="\t", header = T)
dim(second.batch.prs.vars)
second.batch.prs.vars$KEY2 <- paste(second.batch.prs.vars$Chromosome, second.batch.prs.vars$NEW_POS_GRCh38, sep = ":")
second.batch.prs.vars$POS_GRCh38 <- second.batch.prs.vars$NEW_POS_GRCh38 -1
second.batch.prs.vars$bed <- paste0(second.batch.prs.vars$Chromosome,":", second.batch.prs.vars$POS_GRCh38, "-",second.batch.prs.vars$NEW_POS_GRCh38)

write.table(unique(second.batch.prs.vars$bed), "Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/prs/extract_batch3_after_PRS_calculation1.bed", row.names = F, col.names = F, quote = F, sep = "\t")
