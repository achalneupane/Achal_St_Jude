...
metal.SJLIFE <- combined_info
metal.SJLIFE$Gender <- gsub(" ", "_", metal.SJLIFE$Gender)

n_rows <- nrow(metal.SJLIFE)

# Split the data frame into two halves
half_index <- ceiling(n_rows / 2)

# Create a new column with concatenated values
metal.SJLIFE <- metal.SJLIFE %>%
...
metal.SJLIFE$n <- gsub("\\s*\\(.*\\)", "", metal.SJLIFE$n)
colnames(metal.SJLIFE) <- c("MarkerName", "Effect", "StdErr", "P", "N")
write.table(metal.SJLIFE, "Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/ttn_bag3/meta_analysis/stratified_analysis_SJLIFE.txt", col.names  = T, sep = " ", row.names = F, quote = F)
...
## African ancestry ##
######################
AFR_common_Kendrick$CMP <- as.factor(AFR_common_Kendrick$CMP)
