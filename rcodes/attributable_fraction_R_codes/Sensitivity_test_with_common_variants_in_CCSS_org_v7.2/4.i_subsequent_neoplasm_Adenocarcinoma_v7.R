...
Adenocarcinoma <- subneo[grepl("Adenocarcinoma", subneo$diag, ignore.case = T),]
Adenocarcinoma <- setDT(Adenocarcinoma)[,.SD[which.min(gradedt)],by=sjlid][order(gradedt, decreasing = FALSE)]
nrow(Adenocarcinoma)
# 44
# Removing samples with SNs within 5 years of childhood cancer
Adenocarcinoma <- Adenocarcinoma[!Adenocarcinoma$sjlid %in% subneo.within5$sjlid,]
nrow(Adenocarcinoma)
# 19
PHENO.ANY_SN$Adenocarcinoma <- factor(ifelse(!PHENO.ANY_SN$sjlid %in% Adenocarcinoma$sjlid, 0, 1))
table(Adenocarcinoma$diaggrp)
...
ALL.LIFESTYLE$CACO <- factor(ifelse(!ALL.LIFESTYLE$SJLIFEID %in% Adenocarcinoma$sjlid, 0, 1))

## Get date (gradedt) and age at diagnosis of SN
ALL.LIFESTYLE$ANY.SN_gradedate <- Adenocarcinoma$gradedt[match(ALL.LIFESTYLE$SJLIFEID, Adenocarcinoma$sjlid)]
ALL.LIFESTYLE$AGE.ANY_SN <- Adenocarcinoma$AGE.ANY_SN[match(ALL.LIFESTYLE$SJLIFEID, Adenocarcinoma$sjlid)]
...
CROSS_CASES.df <- CROSS_CASES.df[c("Adenocarcinoma", "smoker_never_yn", "smoker_former_or_never_yn", "PhysicalActivity_yn",
                                   "NOT_RiskyHeavyDrink_yn", "HEALTHY_Diet_yn", Not_obese_yn = "Not_obese_yn")]

CROSS_CASES.df <- apply_labels(CROSS_CASES.df, Adenocarcinoma = "Adenocarcinoma", smoker_never_yn = "smoker_never_yn", 
...
as.data.frame(t(CROSS_CASES.df %>%
                  cross_cases(Adenocarcinoma, list(smoker_never_yn , smoker_former_or_never_yn, PhysicalActivity_yn, NOT_RiskyHeavyDrink_yn, HEALTHY_Diet_yn, Not_obese_yn))))
...
###########################
## SJLIFE (ALL) 
mod1 <- glm(Adenocarcinoma ~ AGE_AT_LAST_CONTACT.cs1 + AGE_AT_LAST_CONTACT.cs2 + AGE_AT_LAST_CONTACT.cs3 + AGE_AT_LAST_CONTACT.cs4 + AGE_AT_DIAGNOSIS + gender + maxneckrtdose.category + epitxn_dose_5.category, family = binomial(link = "logit"), data = PHENO.ANY_SN)
summary(mod1)

## SJLIFE (EUR)
mod1.EUR <- glm(Adenocarcinoma ~ AGE_AT_LAST_CONTACT.cs1 + AGE_AT_LAST_CONTACT.cs2 + AGE_AT_LAST_CONTACT.cs3 + AGE_AT_LAST_CONTACT.cs4 + AGE_AT_DIAGNOSIS + gender + maxneckrtdose.category + epitxn_dose_5.category, family = binomial(link = "logit"), data = PHENO.ANY_SN.EUR)
summary(mod1.EUR)

##########################
dat_all = PHENO.ANY_SN
fit_all = glm(formula = Adenocarcinoma ~ Zhaoming_carriers + Qin_without_Zhaoming_vars_carriers + 
...
# 0.549
# 0.55 (Without diet)
# 0.555 [With HEI 2015 (instead of Diet) and other 4 lifestyle factors]
##################
## P/LP and PRS ##
##################
...
# 0.094
# -0.031 (Without diet)
# -0.138 [With HEI 2015 (instead of Diet) and other 4 lifestyle factors]
