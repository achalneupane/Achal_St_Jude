...
## see all age data
age.data <- data[c(1, grep("age" , colnames(data), ignore.case = T))]
...
# 
# cs.data2 <- SNs_py2[c("sjlid", "AGE_AT_LAST_CONTACT.cs1", "AGE_AT_LAST_CONTACT.cs2", "AGE_AT_LAST_CONTACT.cs3", "AGE_AT_LAST_CONTACT.cs4")]
...
# SNs_py2$any_chemo_missing <- apply(SNs_py2[c("cat_epitxn_dose")], 1, function(x) any("Unknown" %in% x))
# SNs_py2$any_chemo_missing  <- factor(ifelse(SNs_py2$any_chemo_missing == FALSE, "No", "Yes"))
# 
# SNs_py2$any_rt_missing <- apply(SNs_py2[c("braincat", "abdcat", "chestcat")], 1, function(x) any("Unknown" %in% x))
# SNs_py2$any_rt_missing  <- factor(ifelse(SNs_py2$any_rt_missing == FALSE, "No", "Yes"))
