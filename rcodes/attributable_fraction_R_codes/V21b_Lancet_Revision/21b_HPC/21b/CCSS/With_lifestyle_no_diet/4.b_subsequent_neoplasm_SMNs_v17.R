...
test.1 <- PHENO.ANY_SN[, c("Smoker_ever_yn_agesurvey", "RiskyHeavyDrink_yn_agesurvey", "Obese_yn_agesurvey", "PhysicalActivity_yn_agesurvey")]
...
test.1 <- PHENO.ANY_SN[, c("Smoker_ever_yn_agesurvey", "RiskyHeavyDrink_yn_agesurvey", "Obese_yn_agesurvey")]
# Apply the function row-wise to the dataframe
test.1$same_count <- apply(test.1, 1, count_same)
table(test.1$same_count)
# 0    1    2    3 
...
# miss_SMN <- get_missing_combinations(PHENO.ANY_SN, columns_to_check)
# 
# miss_SMN <- calculate_missing_counts(PHENO.ANY_SN)
...
# miss_SMN <- calculate_missing_percentages(PHENO.ANY_SN)
...
save.image("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/PHENOTYPE//lancetOncology/ccss.SMNs.V21b_without_diet.Rdata")
