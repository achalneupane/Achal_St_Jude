...
# In V2 version, I am using the reference for lifestyle from previous analysis and expanding the non-reference variables
...
BMI.PA.SMK.DRK <- read.delim("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/PHENOTYPE/CCSS_Data_from_Huiqi/1_22_2025/add2var_race.txt", header = T, sep = "\t", stringsAsFactors = F)
## Read new Physcial activity data
...
keep.reference <- PHENO.ANY_SN.old$ccssid[PHENO.ANY_SN.old$PhysicalActivity_yn == "Yes"]
PHENO.ANY_SN$tmp <- PHENO.ANY_SN$PhysicalActivity_yn
PHENO.ANY_SN$PhysicalActivity_yn [PHENO.ANY_SN$ccssid %in% keep.reference] <- NA


# Calculate the median (excluding NAs)
median_value <- median(PHENO.ANY_SN$PhysicalActivity_yn, na.rm = TRUE)
# Create categorical variable: "Below_Median" and "Above_Median"
PHENO.ANY_SN$PhysicalActivity_yn <- ifelse(PHENO.ANY_SN$PhysicalActivity_yn <= median_value, "1st", "2nd")
# PHENO.ANY_SN$PhysicalActivity_yn <- factor(PHENO.ANY_SN$PhysicalActivity_yn, levels = c("1st", "2nd"))
PHENO.ANY_SN$PhysicalActivity_yn [PHENO.ANY_SN$ccssid %in% keep.reference] <- "Yes"
...
keep.reference <- PHENO.ANY_SN.old$ccssid[PHENO.ANY_SN.old$Smoker_ever_yn == "No"]

# Calculate the median (excluding NAs)
median_value <- median(PHENO.ANY_SN$Smoker_ever_yn, na.rm = TRUE)
# Create categorical variable: "Below_Median" and "Above_Median"
PHENO.ANY_SN$Smoker_ever_yn <- ifelse(PHENO.ANY_SN$Smoker_ever_yn <= median_value, "1st", "2nd")
table(PHENO.ANY_SN$Smoker_ever_yn)
PHENO.ANY_SN$Smoker_ever_yn [PHENO.ANY_SN$ccssid %in% keep.reference] <- "No"
...
PHENO.ANY_SN$Smoker_ever_yn [is.na(PHENO.ANY_SN$Smoker_ever_yn)] <- "Unknown"
table(PHENO.ANY_SN$Smoker_ever_yn)
PHENO.ANY_SN$Smoker_ever_yn <- factor(PHENO.ANY_SN$Smoker_ever_yn, 
                                           levels = c("No", "1st", "2nd", "Unknown"))
...
keep.reference <- PHENO.ANY_SN.old$ccssid[PHENO.ANY_SN.old$RiskyHeavyDrink_yn == "No"]

# PHENO.ANY_SN$RiskyHeavyDrink_yn <- factor(
#   ifelse(PHENO.ANY_SN$RiskyHeavyDrink_yn %in% c(9, 10), "Category_9_10",
...
#   levels = c("Category_9_10", "Category_7_8", "Category_4_6", "Category_1_3", "Unknown")  # Ensures order
# )

PHENO.ANY_SN$RiskyHeavyDrink_yn <- as.numeric(PHENO.ANY_SN$RiskyHeavyDrink_yn)

PHENO.ANY_SN$RiskyHeavyDrink_yn[PHENO.ANY_SN$ccssid %in% keep.reference] <- NA
# Calculate the median (excluding NAs)
median_value <- median(PHENO.ANY_SN$RiskyHeavyDrink_yn, na.rm = TRUE)
# Create categorical variable: "Below_Median" and "Above_Median"
PHENO.ANY_SN$RiskyHeavyDrink_yn <- ifelse(PHENO.ANY_SN$RiskyHeavyDrink_yn <= median_value, "1st", "2nd")
table(PHENO.ANY_SN$RiskyHeavyDrink_yn)

PHENO.ANY_SN$RiskyHeavyDrink_yn [is.na(PHENO.ANY_SN$RiskyHeavyDrink_yn)] <- "Unknown"
PHENO.ANY_SN$RiskyHeavyDrink_yn[PHENO.ANY_SN$ccssid %in% keep.reference] <- "No"

PHENO.ANY_SN$RiskyHeavyDrink_yn <- factor(PHENO.ANY_SN$RiskyHeavyDrink_yn, 
       levels = c("No", "1st", "2nd", "Unknown"))
...
BASALcell.exp <- read.table("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/ccss_exp_wgs/attr_fraction/prs/prs_out/Basal_cell_carcinoma_PGS000119_prs.profile", header = T)
BASALcell.org <- read.table("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/ccss_org_hrc/ccss_org_hrc_vcf_GRCh38/attr_fraction/prs/prs_out/Basal_cell_carcinoma_PGS000119_prs.profile", header = T)
...
save.image("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/PHENOTYPE/lancetOncology/00.CCSS_Genetic_data_P_LP_v17.Rdata")
