...
# table(PHENO.ANY_SN$ccssid == PHENO.ANY_SN.old$ccssid)
PHENO.ANY_SN.old$PhysicalActivity_yn <- as.character(PHENO.ANY_SN.old$PhysicalActivity_yn)

PHENO.ANY_SN$PhysicalActivity_yn[PHENO.ANY_SN.old$PhysicalActivity_yn =="Yes"] <- "Yes"
tmp.df <- PHENO.ANY_SN[PHENO.ANY_SN$PhysicalActivity_yn !="Yes",]
tmp.df$PhysicalActivity_yn <- as.numeric(tmp.df$PhysicalActivity_yn)

## Physical activity tertiles
TERT = unname(quantile(tmp.df$PhysicalActivity_yn[tmp.df$PhysicalActivity_yn !=0], c(1/3, 2/3, 1), na.rm = T))
tmp.df$PhysicalActivity_yn <- cut(tmp.df$PhysicalActivity_yn, 
                                        breaks = c(0, TERT), 
