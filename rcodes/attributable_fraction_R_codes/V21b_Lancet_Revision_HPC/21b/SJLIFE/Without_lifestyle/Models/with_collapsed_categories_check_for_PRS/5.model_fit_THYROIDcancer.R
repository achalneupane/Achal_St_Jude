...
## EUR
fit_all = glm(formula = event ~ Thyroid_PRS.Zscore +
                AGE_AT_LAST_CONTACT.cs1 + AGE_AT_LAST_CONTACT.cs2 + AGE_AT_LAST_CONTACT.cs3 + AGE_AT_LAST_CONTACT.cs4 +
                AGE_AT_DIAGNOSIS + gender + 
                maxneckrtdose.category + epitxn_dose_5.category + 
                any_chemo_missing + any_rt_missing,
              family = "poisson", offset = log(dat_all$PY), data = dat_all, subset = EUR > 0.8)
...
fit_all = glm(formula = event ~ Thyroid_PRS.Zscore +
                ...
                maxneckrtdose.category + epitxn_dose_5.category + 
                any_rt_missing,
              family = "poisson", offset = log(dat_all$PY), data = dat_all, subset = AFR > 0.6)
