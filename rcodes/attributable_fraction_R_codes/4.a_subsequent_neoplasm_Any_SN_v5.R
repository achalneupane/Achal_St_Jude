...
as.data.frame(t(CROSS_CASES.df %>%
  cross_cases(ANY_SN, list(smoker_never_yn , smoker_former_or_never_yn, PhysicalActivity_yn, NOT_RiskyHeavyDrink_yn, HEALTHY_Diet_yn))))
...
## Get predicted values ##
##########################
dat_all$pred_all = predict(fit_all, newdat = subset(dat_all, gender == "Female"), type = "response")
...
dat_tx$maxsegrtdose.category = dat_tx$maxabdrtdose.category =  dat_tx$maxchestrtdose.category = dat_tx$epitxn_dose_5.category = "None"
dat_all$pred_no_tx = predict(fit_all, newdata = subset(dat_tx, gender == "Female"), type = "response")
