# load ANY SN data
load("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/PHENOTYPE/ccss.SMNs.V14.Rdata")

df <- cbind.data.frame(any_lifestyle_missing= PHENO.ANY_SN$any_lifestyle_missing,any_tx_missing= PHENO.ANY_SN$any_tx_missing,SMNs= PHENO.ANY_SN$SMNs)
library(dplyr)

tx <- round(as.numeric(as.data.frame(t(df %>%
                                         group_by(SMNs, any_tx_missing) %>%
                                         dplyr::summarise(n = n()) %>%
                                         group_by(SMNs) %>%
...
lifestyle <- round(as.numeric(as.data.frame(t(df %>%
                                                group_by(SMNs, any_lifestyle_missing) %>%
                                                dplyr::summarise(n = n()) %>%
                                                group_by(SMNs) %>%
                                                dplyr::mutate(percentage = n / sum(n) * 100) %>%
                                                ungroup() %>%
