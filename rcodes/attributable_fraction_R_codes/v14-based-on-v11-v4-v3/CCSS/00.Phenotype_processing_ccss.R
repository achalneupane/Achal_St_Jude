...
lifestyle <- reshape(BMI.PA.SMK.DRK, direction='l', idvar='ccssid', varying=sort(names(BMI.PA.SMK.DRK)[-1]))

lifestyle$age <- as.numeric(lifestyle$age)
...
## Keep adult only
lifestyle <- subset(lifestyle, age >= 18)


# Keep only those that have CCSS WGS
lifestyle <- lifestyle[lifestyle$ccssid %in% ccss_samples,]
...
# 
# 
# bmi <- bmi_iid_dob_18_sorted[c("ccssid", "time", "age", "bmi")]
# bmi$key <- paste(bmi$ccssid, bmi$time, bmi$age, sep = ":")
# 
# met <- MET_iid_dob_18_sorted[c("ccssid", "time", "age", "MET")]
# met$key <- paste(met$ccssid, met$time, met$age, sep = ":")
# 
# drk <- drk_iid_dob_18_sorted[c("ccssid", "time", "age", "riskydrk")]
# drk$key <- paste(drk$ccssid, drk$time, drk$age, sep = ":")
# 
# smk <- smk_iid_dob_18_sorted[c("ccssid", "time", "age", "smk")]
# smk$key <- paste(smk$ccssid, smk$time, smk$age, sep = ":")
# 
# nrow(bmi); nrow(met); nrow(drk); nrow(smk)
# 
# 
# 
# merged_df <- Reduce(function(x, y) left_join(x, y %>% select(-ccssid, -time, -age), by = "key"), list(smk, drk, met, bmi))
# # > dim(merged_df)
# # [1] 47199     8
...
CCSS_data$Not_obese_yn_agesurvey <- as.numeric(bmi_iid_dob_18_uniq$age[match(CCSS_data$ccssid, bmi_iid_dob_18_uniq$ccssid)])
CCSS_data$Not_obese_yn <- factor(ifelse(as.numeric(bmi_iid_dob_18_uniq$bmi[match(CCSS_data$ccssid, bmi_iid_dob_18_uniq$ccssid)]) < 30, 1, 0))
CCSS_data$Not_obese_yn <- factor(CCSS_data$Not_obese_yn, level = c(1, 0, "Unknown")) 
CCSS_data$Not_obese_yn[is.na(CCSS_data$Not_obese_yn)] <- "Unknown"
...
CCSS_data$PhysicalActivity_yn <- factor(MET_iid_dob_18_uniq$CDC[match(CCSS_data$ccssid, MET_iid_dob_18_uniq$ccssid)])
CCSS_data$PhysicalActivity_yn <- ifelse (CCSS_data$PhysicalActivity_yn == "Yes", 1, 0)
CCSS_data$PhysicalActivity_yn[is.na(CCSS_data$PhysicalActivity_yn)] <- "Unknown"
CCSS_data$PhysicalActivity_yn <- factor(CCSS_data$PhysicalActivity_yn, level = c(1, 0, "Unknown")) 

# Smoker
CCSS_data$SMK <- smk_iid_dob_18_uniq$smk[match(CCSS_data$ccssid, smk_iid_dob_18_uniq$ccssid)]
CCSS_data$smoker_former_or_never_yn_agesurvey <- as.numeric(smk_iid_dob_18_uniq$age[match(CCSS_data$ccssid, smk_iid_dob_18_uniq$ccssid)])
CCSS_data$smoker_former_or_never_yn <- factor(smk_iid_dob_18_uniq$smk[match(CCSS_data$ccssid, smk_iid_dob_18_uniq$ccssid)])
CCSS_data$smoker_former_or_never_yn <- factor(ifelse(CCSS_data$smoker_former_or_never_yn != 3, 1, 0))
CCSS_data$smoker_former_or_never_yn <- factor(CCSS_data$smoker_former_or_never_yn, level = c(1, 0, "Unknown")) 
CCSS_data$smoker_former_or_never_yn[is.na(CCSS_data$smoker_former_or_never_yn)] <- "Unknown"

# drinker
CCSS_data$DRK <- drk_iid_dob_18_uniq$riskydrk[match(CCSS_data$ccssid, smk_iid_dob_18_uniq$ccssid)]
CCSS_data$NOT_RiskyHeavyDrink_yn_agesurvey <- as.numeric(drk_iid_dob_18_uniq$age[match(CCSS_data$ccssid, smk_iid_dob_18_uniq$ccssid)])
CCSS_data$NOT_RiskyHeavyDrink_yn <- factor(ifelse(factor(drk_iid_dob_18_uniq$riskydrk[match(CCSS_data$ccssid, smk_iid_dob_18_uniq$ccssid)]) == "No", 1, 0))
CCSS_data$NOT_RiskyHeavyDrink_yn <- factor(CCSS_data$NOT_RiskyHeavyDrink_yn, level = c(1, 0, "Unknown")) 
CCSS_data$NOT_RiskyHeavyDrink_yn[is.na(CCSS_data$NOT_RiskyHeavyDrink_yn)] <- "Unknown"
