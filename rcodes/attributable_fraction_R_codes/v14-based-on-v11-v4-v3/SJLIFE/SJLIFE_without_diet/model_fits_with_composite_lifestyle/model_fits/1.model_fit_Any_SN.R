# load ANY SN data
load("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/PHENOTYPE/6.sjlife_without_diet.Any_SNs.V14-4-3_with_composite_lifestyle.Rdata")

table(PHENO.ANY_SN$any_lifestyle_missing)

df <- cbind.data.frame(any_lifestyle_missing= PHENO.ANY_SN$any_lifestyle_missing,any_tx_missing= PHENO.ANY_SN$any_tx_missing,ANY_SNs= PHENO.ANY_SN$ANY_SNs)
library(dplyr)

tx <- round(as.numeric(as.data.frame(t(df %>%
  group_by(ANY_SNs, any_tx_missing) %>%
  dplyr::summarise(n = n()) %>%
  group_by(ANY_SNs) %>%
...
lifestyle <- round(as.numeric(as.data.frame(t(df %>%
                                   group_by(ANY_SNs, any_lifestyle_missing) %>%
                                   dplyr::summarise(n = n()) %>%
                                   group_by(ANY_SNs) %>%
                                   dplyr::mutate(percentage = n / sum(n) * 100) %>%
                                   ungroup() %>%
...
missing.ANY_SNs <- c(tx, lifestyle)
# 1.33  1.65 35.36 32.67
