# load ANY SN data
load("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/attr_fraction/PHENOTYPE/6.sjlife_without_diet.BREASTcancer.V14-4-3.Rdata")

df <- cbind.data.frame(any_lifestyle_missing= PHENO.ANY_SN$any_lifestyle_missing,any_tx_missing= PHENO.ANY_SN$any_tx_missing,BREASTcancer= PHENO.ANY_SN$BREASTcancer)
library(dplyr)

tx <- round(as.numeric(as.data.frame(t(df %>%
                                         group_by(BREASTcancer, any_tx_missing) %>%
                                         dplyr::summarise(n = n()) %>%
                                         group_by(BREASTcancer) %>%
...
lifestyle <- round(as.numeric(as.data.frame(t(df %>%
                                                group_by(BREASTcancer, any_lifestyle_missing) %>%
                                                dplyr::summarise(n = n()) %>%
                                                group_by(BREASTcancer) %>%
                                                dplyr::mutate(percentage = n / sum(n) * 100) %>%
                                                ungroup() %>%
...
missing.BREASTcancer <- c(tx, lifestyle)
...
dat_plp.prs$Mavaddat_2019_ER_OVERALL_Breast_PRS.tertile.category = "1st"
...
dat_tx.plp.prs.lifestyle$Mavaddat_2019_ER_OVERALL_Breast_PRS.tertile.category = "1st"
