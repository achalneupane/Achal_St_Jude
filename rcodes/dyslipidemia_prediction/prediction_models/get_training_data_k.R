...
analysis_data <- dysl_data %>%
  dplyr::transmute(mrn = mrn, 
                   ...
                   ckd = ckd, htn = htn,
                   anthracyclines = as.numeric(anthracyclines_dose_5 > 0),
                   cisplatin = as.numeric(cisplatin_dose_5 > 0),
                   carboplatin = as.numeric(carboplatin_dose_5 > 0),
                   corticosteroids_5 = corticosteroids_5,
                   maxabdrtdose1 = as.numeric(maxabdrtdose > 0),
                   maxabdrtdose2 = as.numeric(maxabdrtdose > 2400),
                   maxchestrtdose1 = as.numeric(maxchestrtdose > 0),
                   maxchestrtdose2 = as.numeric(maxchestrtdose >= 1500),
                   maxchestrtdose3 = as.numeric(maxchestrtdose >= 3500),
                   maxsegrtdose1 = as.numeric(maxsegrtdose > 0),
                   maxsegrtdose2 = as.numeric(maxsegrtdose >= 1500),
                   maxsegrtdose3 = as.numeric(maxsegrtdose >= 3000),
                   score677, score686, score688, score699, score888,
                   list_a_carrier, list_ab_carrier, list_abc_carrier,
                   time_to_dysl = pmin(time_to_dysl, 10), 
                   dyslipidemia = ifelse(time_to_dysl > 10, 0, dyslipidemia)) 
...
##################
write.csv(clinical_model_data, file = "dyslipidemia_2023.csv", row.names = FALSE)

mydata1 <- read.csv("dyslipidemia_2023.csv")

mydata2 <- read.csv("dyslipidemia_data.csv")

# Merge the two datasets based on the ID column
merged_data <- merge(mydata1, mydata2, by = "sjlid", all.x = TRUE)

# Write the merged dataset to a new file
write.csv(merged_data, "dyslipidemia_prediction_model_data_2023.csv", row.names = FALSE)
