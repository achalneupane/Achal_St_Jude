...
df <- read.xlsx("dyslipidemia_prediction_model_data_2023.xlsx")
...
#count_18.5 <- sum(df1$bmiadj < 18.5, na.rm = TRUE)
#count_18.5 <- sum(df1$bmiadj >= 24.99 & df1$bmiadj < 29.99, na.rm = TRUE)

# Define breaks for the categories
breaks <- c(-Inf, 18.49, 24.99, 29.99, Inf)  # Breaks for the ranges: [0, 5], (5, 10], (10, 15], (15, Inf)
labels = c("<=18.5", "18.5-24.9", "25.0-29.9", ">=30") # Labels for the breaks

# Coerce "ant_count" column to numeric
df0$bmiadj <- as.numeric(df0$bmiadj)
df1$bmiadj <- as.numeric(df1$bmiadj)

# Create categories based on the breaks
df0$categories <- cut(df0$bmiadj, breaks = breaks, labels = labels)
df1$categories <- cut(df1$bmiadj, breaks = breaks, labels = labels)
...
print(percentage_by_category1)

# Create a dataframe with results
results_df_bmiadj <- data.frame(Breaks = labels, 
                                Count_Percentage_bmiadj1 = paste0(as.numeric(count_by_category1), " (", percentage_by_category1, "%)"),
                                Count_Percentage_bmiadj0 = paste0(as.numeric(count_by_category), " (", percentage_by_category, "%)"))
