...
load("results/prediction_model_cv_results.Rdata")
...
auc_clinical_model$Cindex # 0.7179835
...
auc_prs_model$Cindex ## CV method: 0.7504482
...
png("ROC_5yr_03.28.png", width = 480, height = 480)
par(mar = c(5, 5, 4, 1))
plot(x = 1, y = 1, type = "n", 
...
legend("bottomright", col = 1:2, lty = c(3, 1), lwd = 3,
       legend = c("Clinical Model (AUC = 72.8)",
                  "Genetic Model (AUC = 75.5)"), cex = 1.6,
       bty = "n")
dev.off()
compare(prs_model_cd_ROC, clinical_model_cd_ROC) # 0.00605255 
## TODO:
## C/D AUC;


# Export to PDF with higher resolution
pdf("ROC_5yr_high_res_03.28.pdf", width = 12, height = 12)  # Set dimensions in inches
...
legend("bottomright", col = c("blue", 2), lty = c(1, 1), lwd = 3,
       legend = c("Clinical Model (AUC = 0.73)",
                  paste("Clinical Model + PRSs (AUC = 0.76)\n", "p = 0.006")), cex = 2.3,  # Adjust legend font size
...
clinical_risk_group <- factor(cut(clinical_model_marker, 
                                  quantile(clinical_model_marker, c(0, 0.35, 0.85, 1)),
                                  include.lowest = T, right = F),
                              labels = c("Low risk", "Medium risk", "High risk"))

prs_risk_group <- factor(cut(prs_model_marker, 
                             quantile(prs_model_marker, c(0, 0.35, 0.85, 1)),
                             include.lowest = T, right = F),
                         labels = c("Low risk", "Medium risk", "High risk"))
...
png("km_risk_stratification_03.28.png", width = 2400, height = 2400, res = 300)
cols <- c("green", "blue", "red")
par(mar = c(5, 5, 4, 1))
plot(km_clinical, col = cols, fun = "event", 
     lty = 2, lwd = 2, ylim = c(0, 0.5),
...
legend("topleft", 
       ...
       #legend = c("Low risk\n", "p = 0.67", "Moderate risk", "High risk"),
       fill = cols,
       bty = "n", cex = 1.8)
legend(x = 4.3, y = 0.5,
       lwd = 3, lty = c(1, 2),
...
pdf("Risk_stratification_high_res_03.28.pdf", width = 12, height = 12)  # Set dimensions in inches
par(mar = c(5, 5, 4, 1))
plot(km_clinical, col = cols, fun = "event", 
     lty = 2, lwd = 3, ylim = c(0, 0.5),
     xaxs = "S", xlab = "Time (years)", ylab = "Cumulative risk",
...
clinical_marker_cutoff <- quantile(clinical_model_marker, c(0, 0.35, 0.85, 1))
prs_marker_cutoff <- quantile(prs_model_marker, c(0, 0.35, 0.85, 1))
