...
plink1.9 --bfile ${BFILE}-HAPMAP-MERGED2 --extract SNPs-2extractandrename.txt --keep-allele-order --make-bed --out ${BFILE}-HAPMAP-MERGED2-SNPSextracted
...
colnames(PED) <- gsub("REF_", "", colnames(PED))


# Now keep only those samples listed by first getting their MRNs
patient_MRN <- read.table("Patient_list_BRIGHT_with_DTI.txt", header = T)

# Read MRN to SJLID
MRN_SJLID <- read.delim("Z:/ResearchHome/Groups/sapkogrp/projects/SJLIFE_WGS/common/sjlife/PHENOTYPE/MRN_SJLID_05_02_2022.csv", sep ="\t", stringsAsFactors = F)
MRN_SJLID <- MRN_SJLID[MRN_SJLID$mrn %in% patient_MRN$ID,]

FINAL.PED <- cbind.data.frame(MRN_SJLID, PED[match(MRN_SJLID$sjlid, PED$IID),])
write.table(FINAL.PED, "Final_PED_43SNPs_153_samples.txt", sep = "\t", row.names = F, col.names = T, quote = FALSE)
