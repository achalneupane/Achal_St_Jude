...
## Create properties
properties <- as.data.frame(colnames(counts2))
colnames(properties) <- "samples"

properties$Project <- ifelse(grepl("Pr1", properties$samples), "Pr1", "Pr2")
properties$samples.Pr <- properties$samples
properties$samples <- gsub("Pr1_|Pr2_", "", properties$samples)
properties$Cell_type [grepl("ISO", properties$samples, ignore.case = T)] <- "ISO"
properties$Cell_type [grepl("ROBO1", properties$samples, ignore.case = T)] <- "ROBO1"
properties$Cell_type [grepl("ROBO2", properties$samples, ignore.case = T)] <- "ROBO2"
properties$Cell_type [grepl("SLIT2", properties$samples, ignore.case = T)] <- "SLIT2"
properties$Dox_treatment <- ifelse(grepl("DOX|DX", properties$samples, ignore.case = T), "Yes", "No")
...
## only cardiomyocyte
properties <- properties[properties$Cell_Type == "Cardiomyocytes",]
...
tiff(
  file = "PCA_LVNC_Project_top_100_genes_Cardiomyocytes.tiff",
  width = 16,
  height = 14,
  units = 'cm',
  res = 300,
...
tiff(file="Heatmap_LVNC_Project_top_500_genes_Cardiomyocytes.tiff",width = 18, height = 32, units = 'cm', res = 300, compression = 'lzw')
phm
dev.off()
```
...
write.table(deseq_results, file="LVNC_project_deseq_results_Cardiomyopathy_vs_Control_Cardiomyocytes.txt", sep="\t")
...
tiff(file="Volcano_plot_Cardiomyopathy_vs_Control_Cardiomyocytes.tiff",
