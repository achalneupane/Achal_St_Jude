...
setwd("Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/gwas/rare_variants/geneBased_June_24_2024/")
#########
## EUR ##
#########
## Phenotype data for GWAS
pheno_gwas = read.table('Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/gwas/pheno/sjlife_eur_dox_only_pcs.pheno', header = TRUE)

## Echo measures with data from most recent visit
echo_data = read.table('Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/echo_data/SJLIFE_echo_measures_most_recent_visit.txt', header = TRUE)

## Merge both data
pheno_final = merge(pheno_gwas, echo_data, by.x = 'FID', by.y = 'sjlid')

## Genotypes of TTN and BAG3 top SNPs
bag3 = read.table('Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/gwas/rare_variants/geneBased_June_24_2024/chr16_25611595_C_T_EUR.dat_recodeA.raw', header = TRUE)
bag3[c(2:6)] = NULL
geno = bag3
...
for (p in 1:length(echo_phenotypes)){
  ...
  if (!(echo_phenotypes[p] == "LVMass2D_Index" | echo_phenotypes[p] == "LA_Volume_Index"| 
    ...
    fit = lm(scale(dat_eff[,echo_phenotypes[p]]/BodySurfaceArea)~chr16.25611595.C.T_T+agedx+agelstcontact+gender+anthra_jco_dose_any+
               PC1+PC2+PC3+PC4+PC5,data=dat_eff)
  } else {
    fit = lm(scale(dat_eff[,echo_phenotypes[p]])~chr16.25611595.C.T_T+agedx+agelstcontact+gender+anthra_jco_dose_any+
               PC1+PC2+PC3+PC4+PC5,data=dat_eff)
  }
...
write.table(res_bag3, "Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/gwas/chr16.25611595.C.T_T_wrt_echo_EUR.txt", col.names = T, row.names = F, quote = F, sep = "\t")

#########
## AFR ##
#########
## Phenotype data for GWAS
pheno_gwas = read.table('Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/gwas/pheno/sjlife_afr_dox_only_pcs.pheno', header = TRUE)
...
## Genotypes of TTN and BAG3 top SNPs
bag3 = read.table('Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/gwas/rare_variants/geneBased_June_24_2024/chr16_25611595_C_T_AFR.dat_recodeA.raw', header = TRUE)
bag3[c(2:6)] = NULL
geno = bag3
...
write.table(res_bag3, "Z:/ResearchHome/Groups/sapkogrp/projects/Cardiotoxicity/common/gwas/chr16.25611595.C.T_T_wrt_echo_AFR.txt", col.names = T, row.names = F, quote = F, sep = "\t")
