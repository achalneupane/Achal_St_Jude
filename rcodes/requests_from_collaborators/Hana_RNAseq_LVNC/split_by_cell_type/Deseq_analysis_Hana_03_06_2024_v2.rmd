---
title: "RNAseq analysis for LVNC project"
...
deseq_results$expression = ifelse(
  deseq_results$padj < 0.05 &
    (deseq_results$log2FoldChange) >= (2),
  "Up",
  ifelse(
    deseq_results$padj < 0.05 & (deseq_results$log2FoldChange) <= (-2),
    "Down",
    "Stable"
  )
)
...
deseq_results_lvnc_vs_others$expression = ifelse(
  deseq_results_lvnc_vs_others$padj < 0.05 &
    (deseq_results_lvnc_vs_others$log2FoldChange) >=  (2),
  "Up",
  ifelse(
    deseq_results_lvnc_vs_others$padj < 0.05 &
      (deseq_results_lvnc_vs_others$log2FoldChange) <= (-2),
...
deseq_results_lvnc_vs_DCM$expression = ifelse(
  deseq_results_lvnc_vs_DCM$padj < 0.05 &
    (deseq_results_lvnc_vs_DCM$log2FoldChange) >=  (2),
  "Up",
  ifelse(
    deseq_results_lvnc_vs_DCM$padj < 0.05 &
      (deseq_results_lvnc_vs_DCM$log2FoldChange) <= (-2),
...
# Write results to a file
write.table(deseq_results_lvnc_vs_DCM, file = "LVNC_project_deseq_results_LVNC_vs_DCM.txt", sep = "\t")
...
# Save the plot as a TIFF file
tiff(file = "Volcano_plot_LVNC_vs_DCM.tiff",
     width = 18, height = 16, units = 'cm', res = 300, compression = 'lzw')
print(p_lvnc_vs_DCM)
dev.off()
```
...
deseq_results_lvnc_vs_HCM$expression = ifelse(
  deseq_results_lvnc_vs_HCM$padj < 0.05 &
    (deseq_results_lvnc_vs_HCM$log2FoldChange) >=  (2),
  "Up",
  ifelse(
    deseq_results_lvnc_vs_HCM$padj < 0.05 &
      (deseq_results_lvnc_vs_HCM$log2FoldChange) <= (-2),
...
# Write results to a file
write.table(deseq_results_lvnc_vs_HCM, file = "LVNC_project_deseq_results_LVNC_vs_HCM.txt", sep = "\t")
...
# Save the plot as a TIFF file
tiff(file = "Volcano_plot_LVNC_vs_HCM.tiff",
     width = 18, height = 16, units = 'cm', res = 300, compression = 'lzw')
print(p_lvnc_vs_HCM)
dev.off()
```