...
## Get same variants if extracted by positions
# setwd("Z:/ResearchHome/Groups/sapkogrp/projects/Genomics/common/MERGED_sjlife1_2_PreQC/cleaned/annotation/snpEff/yadav_dox_abstract_09_04_2024/")
# df <- fread("extracted_positions_chr10_and_13.txt", header = T, sep = "\t")
# df$ID <- sub(";.*", "", df$ID)
# df$CHROM <- sub(".*:", "", df$CHROM)
# df <- as.data.frame(df)
...
genes.31 <- c("ADD3", "DUSP5", "RBM20", "PDCD4", "BBIP1", "SHOC2", "ADRA2A", 
              "ACSL5", "VTI1A", "TCF7L2", "NRAP", "CASP7", "NHLRC2", "ADRB1", 
              "AFAP1L2", "ABLIM1", "GFRA1", "PNLIPRP2", "HSPA12A", "SLC18A2", 
              "PDZD8", "RAB11FIP2", "MYCBP2", "SLAIN1", "EDNRB", "GPC6", "TGDS", 
              "GPR180", "ABCC4", "UGGT2", "MBNL2")

pp <- cbind.data.frame(df.nonsynonymous.unique$ID,df.nonsynonymous.unique$ANN....GENE, df.nonsynonymous.unique$CLNSIG)

table(df.nonsynonymous$CHROM)

vars.counts <- as.data.frame(table(df.nonsynonymous.unique$ANN....GENE))
vars.counts$CHROM <- df$CHROM[match(vars.counts$Var1, df$ANN....GENE)]
vars.counts <- cbind.data.frame(CHROM=vars.counts$CHROM, GENE=vars.counts$Var1, Variants=vars.counts$Freq)
##########
## P/LP ##
##########
...
# rm(df)
# rm(raw)
# save.image("yadav_dox_abstract_09_04_2024_both_chr10_and_chr13.Rdata")

load("yadav_dox_abstract_09_04_2024_both_chr10_and_chr13.Rdata")

table(df.nonsynonymous.unique$CHROM)
# chr10 chr13 
# 876   426
# Since they are in two chrmosomes only, we can analyze CHR10 and chr13
table(df.nonsynonymous.unique$ANN....GENE)

# raw.nonsynonymous[raw.nonsynonymous == 2] <- 1
# raw.nonsynonymous$varSum <- NA
# raw.nonsynonymous$carrier <- NA
# raw.nonsynonymous$varSum <- rowSums(raw.nonsynonymous, na.rm = T)
# raw.nonsynonymous$carrier <- ifelse (raw.nonsynonymous$varSum > 0, 1,0)
...
pheno.sjlife$carrier_nonsynonymous <- factor(pheno.sjlife$carrier_nonsynonymous, levels = c(0, 1))
###########################
## Fisher test CMP2 plus ##
###########################
test <- fisher.test(table(pheno.sjlife$CMP2plus, pheno.sjlife$carrier_nonsynonymous))
test
...
###########################
test <- fisher.test(table(pheno.sjlife$CMP3plus, pheno.sjlife$carrier_nonsynonymous))
test
# Fisher's Exact Test for Count Data
...
## Fisher test CMP2 plus ##
###########################
test <- fisher.test(table(pheno.sjlife.EUR$CMP2plus, pheno.sjlife.EUR$carrier_nonsynonymous))
test
# Fisher's Exact Test for Count Data
...
## Fisher test CMP3 plus ##
###########################
test <- fisher.test(table(pheno.sjlife.EUR$CMP3plus, pheno.sjlife.EUR$carrier_nonsynonymous))
test
# Fisher's Exact Test for Count Data
...
## Fisher test CMP2 plus ##
###########################
test <- fisher.test(table(pheno.sjlife.AFR$CMP2plus, pheno.sjlife.AFR$carrier_nonsynonymous))
test
# Fisher's Exact Test for Count Data
...
## Fisher test CMP3 plus ##
###########################
test <- fisher.test(table(pheno.sjlife.AFR$CMP3plus, pheno.sjlife.AFR$carrier_nonsynonymous))
test
# Fisher's Exact Test for Count Data
