## This is the revision after the comment from Yutaka on 05/20/2024
...
dim(SERUM.not.CA171)
# 7493
SERUM.not.CA171 <- SERUM.not.CA171[!is.na(SERUM.not.CA171$diaggrp),]
dim(SERUM.not.CA171)
# 7490
...
## Check if all 200 Zhaomings are present
HL.210.zhaoming <- read.table("Z:/ResearchHome/ClusterHome/aneupane/data/Yadav_serum/v12_output/HL.selected.210.txt", header = F)
table(HL.210.zhaoming$V1 %in% all.hodgkin$sjlid)
# FALSE  TRUE 
# 25   185   ## only 185 of the 210 are present in the eligible HL samples
all.hodgkin.extract <- all.hodgkin[all.hodgkin$sjlid %in% HL.210.zhaoming$V1,]
## Get the first sample age
all.hodgkin.extract <- all.hodgkin.extract %>%
  arrange(sjlid, Sample_age) %>%  # Sort by sjlid and ageevent
  group_by(sjlid) %>%              # Group by sjlid
  slice(1) 

dim(all.hodgkin.extract)
# 185  10
all.hodgkin.extract$selection_group <- "185_Hodgkin_lymphoma"
...
all.non.hodgkin.extract <- all.non.hodgkin[all.non.hodgkin$sjlid %in% all.non.hodgkin.extract,]
## Get the first sample age
all.non.hodgkin.extract <- all.non.hodgkin.extract %>%
  arrange(sjlid, Sample_age) %>%  # Sort by sjlid and ageevent
  group_by(sjlid) %>%              # Group by sjlid
  slice(1) 
...
#################################
all.wanted.df.1185 <- rbind.data.frame(CA.171, all.hodgkin.extract, all.non.hodgkin.extract, first_ageatsample.zhaoming.100)
dim(all.wanted.df.1185)
# 1185    9

write.table(all.wanted.df.1185, "Z:/ResearchHome/ClusterHome/aneupane/data/Yadav_serum/v12_output/plasma_data_batch1_1185_samples_v3.txt", col.names = T, row.names = F, quote = F, sep = "\t", na="")
...
all.wanted.df.1185.to.update <- read.table("Z:/ResearchHome/ClusterHome/aneupane/data/Yadav_serum/v12_output/plasma_data_batch1_1185_samples_v3.txt", header = T, sep = "\t")
all.wanted.df.1185.to.update$Sex <- demographic$gender[match(all.wanted.df.1185.to.update$sjlid, demographic$sjlid)]
all.wanted.df.1185.to.update$racegrp <- demographic$racegrp[match(all.wanted.df.1185.to.update$sjlid, demographic$sjlid)]
write.table(all.wanted.df.1185.to.update, "Z:/ResearchHome/ClusterHome/aneupane/data/Yadav_serum/v12_output/plasma_data_batch1_1185_samples_v3_updated.txt", col.names = T, row.names = F, quote = F, sep = "\t", na="")

all.wanted.df.1185.to.update.proteomics <- all.wanted.df.1185.to.update[c("tb_number", "Sample_age", "selection_group", "Sex", "racegrp")]
#############################
## Give this to proteomics ##
#############################
write.table(all.wanted.df.1185.to.update.proteomics, "Z:/ResearchHome/ClusterHome/aneupane/data/Yadav_serum/v12_output/plasma_data_batch1_1185_samples_to_proteomics_core.txt", col.names = T, row.names = F, sep = "\t", quote = F)
table(all.wanted.df.1185.to.update$num_vials, all.wanted.df.1185.to.update$selection_group)
...
##############################
all.wanted.df.1185.to.update.kyla <- all.wanted.df.1185.to.update[c("tb_number", "sjlid", "Sample_age", "num_vials", "vitalstatus","selection_group")]
write.table(all.wanted.df.1185.to.update.kyla, "Z:/ResearchHome/ClusterHome/aneupane/data/Yadav_serum/v12_output/plasma_data_batch1_1185_samples_to_ECC.txt", col.names = T, row.names = F, sep = "\t", quote = F)
...
## add total num_vials ##
#########################
all.wanted.df.1185.to.update.kyla$total_vials_for_all_visits <- total_vials.by.sjlid$num_vials[match(all.wanted.df.1185.to.update.kyla$sjlid, total_vials.by.sjlid$sjlid)]
all.wanted.df.1185.to.update.kyla$KEY <- paste0(all.wanted.df.1185.to.update.kyla$sjlid, ":", all.wanted.df.1185.to.update.kyla$Sample_age)

all.wanted.df.1185.to.update.kyla$total_plasma_for_the_visit <- total_plasma.vials.by.sjlid$num_vials[match(all.wanted.df.1185.to.update.kyla$KEY, total_plasma.vials.by.sjlid$KEY)]
all.wanted.df.1185.to.update.kyla$total_serum_for_the_visit <- total_serum.vials.by.sjlid$num_vials[match(all.wanted.df.1185.to.update.kyla$KEY, total_serum.vials.by.sjlid$KEY)]

all.wanted.df.1185.to.update.kyla$total_plasma_for_the_visit[is.na(all.wanted.df.1185.to.update.kyla$total_plasma_for_the_visit)] <- 0
all.wanted.df.1185.to.update.kyla$total_serum_for_the_visit[is.na(all.wanted.df.1185.to.update.kyla$total_serum_for_the_visit)] <- 0
all.wanted.df.1185.to.update.kyla$visit_depleted_YN <- ifelse((all.wanted.df.1185.to.update.kyla$total_plasma_for_the_visit + all.wanted.df.1185.to.update.kyla$total_serum_for_the_visit) >= 2, "No", "Yes")
write.table(all.wanted.df.1185.to.update.kyla, "Z:/ResearchHome/ClusterHome/aneupane/data/Yadav_serum/v12_output/plasma_data_batch1_1185_samples_to_ECC_with_vials_info.txt", col.names = T, row.names = F, sep = "\t", quote = F)
...
new.missing$selection_group <- all.wanted.df.1185.to.update$selection_group[match(new.missing$tb_number, all.wanted.df.1185.to.update$tb_number)]
