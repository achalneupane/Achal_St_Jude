...
cc <- pbmc_markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
cc
```
...
SaveFigure(pbmc_markers_featurePLT, "pbmc_markers", width = 30, height = 30)
```
...
```{r, echo=TRUE}
FeaturePlot(pbmc, features = c("CD3D", "GNLY", "IFI6"), split.by = "stim", max.cutoff = 3,
    cols = c("grey", "red"))
```
