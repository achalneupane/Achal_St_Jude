---
title: "scRNAseq analysis with Seurat (Cardiomyopathy vs LVNC)"
...
## For question 2, we will remove controls from the analysis and only keep three cardiomyopathy samples
ifnb.list$S_25 <- NULL
...
```{r, echo = TRUE}
DefaultAssay(immune.combined) <- "RNA"
FeaturePlot(immune.combined, features = top.markers, min.cutoff = "q9")
```
...
DimPlot(immune.combined, label = TRUE, repel = TRUE)
```
...
DotPlot(immune.combined, features = markers.to.plot, cols = c("blue", "red"), dot.scale = 8, split.by = "grouping_var1") +
    RotatedAxis()
...
```{r, echo = TRUE}
FeaturePlot(immune.combined, features = genes.to.label, split.by = "grouping_var1", max.cutoff = 3,
    cols = c("grey", "red"))
