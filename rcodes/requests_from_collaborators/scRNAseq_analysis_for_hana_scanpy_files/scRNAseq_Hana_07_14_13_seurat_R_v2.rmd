...
pbmc$grouping_var2[!grepl("GW64|S_25",pbmc$sample)] <- "other_cardiomypathy"
```
...
plot
SaveFigure(plot, "pc1_2_scatter_control_vs_cardiomyopathy", width = 8, height = 6) # **
```
...
plot1
plot2 <- DimPlot(pbmc, reduction = "umap", label = TRUE, repel = TRUE)
plot2
SaveFigure(plot1, "umap_louvain_res_p3", width = 9, height = 8)
SaveFigure(plot2, "umap_louvain_res_p3_all", width = 9, height = 8)
```
...
```{r, echo=TRUE}
DimPlot(pbmc, reduction = "umap", split.by = "grouping_var1")
```
...
```{r, echo=TRUE}
# Idents(pbmc) <- "grouping_var1"
# pbmc_markers <- FindMarkers(pbmc, ident.1 = "cardiomyopathy", ident.2 = "control", min.pct = 0.25, logfc.threshold = 0.25)
# pbmc_markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)

```


We can explore these marker genes between subgroups.
```{r, echo=TRUE}
# loop over clusters
...
pbmc_markers <- FindMarkers(pbmc.tmp, ident.1 = "control", ident.2 = "cardiomyopathy", min.pct = 0.25, logfc.threshold = 0.25)
pbmc_markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
pbmc_markers_sorted <- pbmc_markers[order(-pbmc_markers$avg_log2FC), ]
```
...
<!-- pbmc_markers <- FindConservedMarkers(pbmc, ident.1 = i, grouping.var = "grouping_var1", verbose = FALSE) -->
<!-- # pbmc_markers <- FindAllMarkers(pbmc, min.pct = 0.25, logfc.threshold = 0.25) -->
<!-- # pbmc_markers <- FindMarkers(pbmc, ident.1 = "control", ident.2 = "cardiomyopathy", min.pct = 0.25, logfc.threshold = 0.25) -->
<!-- # pbmc_markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC) -->
<!-- top.markers.tmp <- rownames(head(pbmc_markers,1)) -->
<!-- top.markers <- c(top.markers, top.markers.tmp) -->
<!-- top.markers.df.tmp <- pbmc_markers -->
<!-- top.markers.df.tmp$cluster <- i -->
<!-- top.markers.df[[i]] <- top.markers.df.tmp -->
<!-- } -->
<!-- # > top.markers -->
<!-- c('CFH','VCAN','MEGF11','ST8SIA4','COL9A2','CSMD3','FGR','NEBL','ANLN','TTN','FBN2','SLC7A11','LDB3','ANLN','KIF20A') -->
