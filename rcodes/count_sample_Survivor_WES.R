...
CCSS <- as.data.frame(all.samples$V2[grepl("CCSS", all.samples$V1)])
## Not sure about these two samples in CCSS, so did not include them
...
SJLIFE_survivor <- as.data.frame(all.samples$V2[all.samples$V2 %in% pop.survivor$sjlid ])
dim(SJLIFE_survivor)
# [1] 4568    1
...
SJLIFE_control <- as.data.frame(all.samples$V2[all.samples$V2 %in% pop.survivor.control$sjlid ])
dim(SJLIFE_control)
# [1] 451   1
