...
awk '{cmd="echo "$0" | sed -e '\''s/.*CCSS-//g; s/^0\\+\\([^0]\\)/\\1/g'\''"; cmd | getline result; close(cmd); print $0"\t"result}' Survivor_WES.samplelist.ccss  > ./biallelic2_noVQSR/rename_ccss.txt
...
bsub \
        -P "${CHR}_biallelic" \
        -J "${CHR}_biallelic" \
        -o "${WORKDIR}/biallelic2_noVQSR/logs/${VCF%.vcf*}_biallelic.%J" \
        -n ${THREADS} \
        -R "rusage[mem=8192]" \
        "/research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic2_noVQSR/biallelic_renaming_split.sh"; \
done;
...
module load tabi
vcftools --gzvcf "${WORKDIR}/${VCF}" --chr ${CHR} --minGQ 20 --min-meanDP 10 --recode --stdout | bgzip > "${WORKDIR}/biallelic2_noVQSR/$(basename ${VCF} .vcf.gz)_tmp0.vcf.gz"
# ## Normalize and make biallelic
bcftools norm -m-any --check-ref -w -f /research/rgs01/reference/public/genomes/Homo_sapiens/GRCh38/GRCh38_no_alt/GCA_000001405.15_GRCh38_no_alt_analysis_set.fa "${WORKDIR}/biallelic2_noVQSR/$(basename ${VCF} .vcf.gz)_tmp0.vcf.gz" -Oz -o "${WORKDIR}/biallelic2_noVQSR/$(basename ${VCF} .vcf.gz)_tmp.vcf.gz"
bcftools annotate --set-id '%CHROM\:%POS\:%REF\:%FIRST_ALT' "${WORKDIR}/biallelic2_noVQSR/$(basename ${VCF} .vcf.gz)_tmp.vcf.gz" -Oz -o "${WORKDIR}/biallelic2_noVQSR/$(basename ${VCF} .vcf.gz)_biallelic.vcf.gz"
bcftools index -f -t --threads 4 "${WORKDIR}/biallelic2_noVQSR/$(basename ${VCF} .vcf.gz)_biallelic.vcf.gz"
...
## Remove LCR region
cd /research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic2_noVQSR/plink_all
...
bsub \
        -P "${CHR}_plk" \
        -J "${CHR}_plk" \
        -o "${WORKDIR}/biallelic2_noVQSR/plink_all/logs/${VCF%.vcf*}_biallelic.%J" \
        -n ${THREADS} \
        -R "rusage[mem=8192]" \
        "./plinkQC.sh"; \
done;
...
plink --vcf "${WORKDIR}/biallelic2_noVQSR/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.vcf.gz" --double-id --vcf-half-call m --keep-allele-order --threads ${THREADS} --make-bed --out "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic"
plink --bfile "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic" --geno 0.10 --keep-allele-order --make-bed --out "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.geno.0.1"
# Filter based on HWE p-value (<1x10^-15)
plink --bfile "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.geno.0.1" --hwe 1e-15 --keep-allele-order --make-bed --out "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.geno.0.1.hwe.1e-15"
...
plink --bfile "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.geno.0.1.hwe.1e-15" --exclude range /research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic/plink_all/LCR-hs38.bed --keep-allele-order --make-bed --out "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.geno.0.1.hwe.1e-15.LCR.removed"
# Calculate MAC
module load plink/2.0
plink2 --bfile "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.geno.0.1.hwe.1e-15.LCR.removed" --mac 1 --keep-allele-order --make-bed --out "${WORKDIR}/biallelic2_noVQSR/plink_all/${CHR}.Survivor_WES.GATK4180.hg38_biallelic.geno.0.1.hwe.1e-15.LCR.removed.MAC.ge.1"
...
ln -s /research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic2_noVQSR/*_biallelic.vcf.gz .
ln -s /research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic2_noVQSR/*_biallelic.vcf.gz.tbi .
...
cd /research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic2_noVQSR/

#!/bin/bash
module load bcftools
# Directory containing your VCF files
vcf_directory="/research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/Survivor_WES/biallelic2_noVQSR/"
