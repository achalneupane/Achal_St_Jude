...
#######################################################################################################################################
#######################################################################################################################################
## Extract rare P/LP from QCed data which did not go through GQ, DP and VQSR filter
...
cd /research_jude/rgs01_jude/groups/sapkogrp/projects/Genomics/common/sjlife/MERGED_SJLIFE_1_2/MERGED_SJLIFE_PLINK_PER_CHR/tasks
plink --bfile ../MERGED.SJLIFE.1.2.GATKv3.4.VQSR.chr3.PASS.decomposed --extract extract_plp_from_clinvar_high_prevalence_WES.txt --make-bed -out chr3PLP
plink --bfile ../MERGED.SJLIFE.1.2.GATKv3.4.VQSR.chr7.PASS.decomposed --extract extract_plp_from_clinvar_high_prevalence_WES.txt --make-bed -out chr7PLP
plink --bfile ../MERGED.SJLIFE.1.2.GATKv3.4.VQSR.chr13.PASS.decomposed --extract extract_plp_from_clinvar_high_prevalence_WES.txt --make-bed -out chr13PLP
plink --bfile chr3PLP --merge-list merge_list --make-bed --out merged_data
plink --bfile merged_data --recodeA --out merged_data_recodeA